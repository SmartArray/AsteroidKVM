...
./scripts/test.sh --ui-hardware # Native UI plus live hardware fullscreen; same session file
...
Local protocol tests start a loopback server on a random port. The video E2E uses a genuine native WebRTC sender and production receiver; the fixture implements the Janus envelope. Session tests substitute only media lifecycle when testing focus and reconnection. Hardware tests do not reboot or log out the supplied token. Text is sent only with the explicit `COMET_E2E_TYPING=1` opt-in; this writes test lines to the focused remote editor.
