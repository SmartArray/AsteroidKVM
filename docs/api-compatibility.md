...
Writes are serialized, never automatically retried, and verified by readback. Readback verifies firmware's stored EDID, not the target OS's active desktop mode. Firmware may normalize a single-block EDID internally; restoration submits the exact previously returned document. Local backups contain monitor data only and are stored under `Application Support/AsteroidKVM/EDIDBackups` with a hashed connection/endpoint/certificate key. AsteroidKVM imports an existing CometKVM backup when that device is first opened.
