...
Real Windows verification found modifier corruption when mapped events were sent in bursts and when HTTP paste used the firmware's fast rate. The production FIFO defaults to waiting 120 ms after each mapped event (configurable per connection from 0 to 1,000 ms in Keyboard & Clipboard settings), and HTTP paste requests `slow=true`. Physical input retains its natural event timing. At the default interval, mapped output is capped near 8.3 scalars/second; use Paste for larger text. The test sends through this production pacing, verifies every mapped reply, and checks unique markers in the returned video with Vision.
